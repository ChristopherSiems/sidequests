...
export default function ExpandedQuestCard({quest, directionsMap}: ExpandedQuestCardProps) {
    return (
        <div className="flex flex-col items-center justify-center border-2 border-gray-300 rounded-md p-4 w-[300px] h-[500px]">
            ...
            {directionsMap}
            <h1 className="mt-2">Event Duration: {formatTime(quest.end_time - quest.start_time )}</h1>
            {quest.link && <a href = {quest.link} className="w-[200px] text-center p-2 rounded-md text-white bg-blue-500 mt-2"> View External Source</a>}
