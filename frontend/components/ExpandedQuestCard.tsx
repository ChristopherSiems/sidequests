...
export default function ExpandedQuestCard({quest, directionsMap}: ExpandedQuestCardProps) {
    ...
    return (
        <div className="flex flex-col items-center justify-center border-2 border-gray-300 rounded-md p-4 w-[300px] h-[500px]">
            ...
            {directionsMap}<h1 className="mt-2">{typeof quest.end_time === "string"? "Time:" : "Duration:"} {typeof quest.end_time === "number" ? formatTime(duration ) :typeof quest.end_time === "string" ? quest.start_time + " to " + quest.end_time : "Open All Day"}</h1>
